...
export default class FavoritePiuService {
  ...
  public async execute({ piu_id, user_id }: IRequest): Promise<void> {
    ...
    if (piuAlreadyFavorite === -1) throw new AppError('Piu already unfavorited');

    const userNewFavorites = user.favorites.filter((favorite) => favorite.id !== piu.id);

    user.favorites = userNewFavorites;
